...
if not st.session_state.logged_in:
    ...
    usuario_opcion = st.selectbox(
        "👤 Selecciona tu usuario",
        ["", "pachuca", "sanmartin", "admin"]
    )

    password = ""
    if usuario_opcion == "admin":
    ...
    if st.button("Ingresar", use_container_width=True):
        ...
        elif usuario_opcion in ["pachuca", "sanmartin"]:
            st.session_state.logged_in = True
            st.session_state.username = usuario_opcion
            st.session_state.is_admin = False
            st.success(f"✅ Bienvenida oficina {usuario_opcion.capitalize()}")
        else:
...
# 📊 Pantalla principal (ya logueado)
    ...
    if not st.session_state.is_admin:
        ...
        if not df.empty:
            st.markdown(f"### 📂 Registros de la oficina **{st.session_state.username.capitalize()}**")
