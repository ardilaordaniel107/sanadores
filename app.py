        ...
        try:
            ...
            if not st.session_state.is_admin:
                response = supabase.table("registros").select(
                    "id, oficina, consultas, controles, ingreso, semana, fecha_registro"
            ...
            st.error("❌ Error al recuperar registros desde Supabase")
            st.exception(e)
    else:
        st.error("⚠ Usuario no definido en la sesión.")
